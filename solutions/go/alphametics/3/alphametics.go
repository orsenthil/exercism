...
// parsePuzzle parses the puzzle string into an equation.
func parsePuzzle(puzzle string) equation {
	...
	for _, item := range strings.Fields(puzzle) {
		if item == "==" {
			...
			// addend is a struct that contains the letters and the number
			// lhs is a slice of addends
			// this statement adds the addend to the lhs slice
